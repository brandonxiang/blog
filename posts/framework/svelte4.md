...
description: Svelte 笔记第四篇，解析 Svelte compiler 的 parse 和 compile 流程，说明模板语法如何转成可执行代码。
---

- [Svelte 笔记三：runtime 源码解读](https://brandonxiang.top/blog/framework-svelte3)
- [Svelte 笔记四：compile 源码解析](https://brandonxiang.top/blog/framework-svelte4)
...
svelte 的 script 被分为 instance 和 module 两种。instance 指的是[svelte runtime 源码解析](https://brandonxiang.top/blog/framework-svelte3)中提及的 instance 方法，是用于处理组件内部的变化逻辑。module 则是处理组件外部的逻辑，它 export 出来的方法可以被组件外部调用。例如下面这个例子 stopAll 这个方法可以被组件外部调用，用于暂停组件的音乐播放器。
...
[《Svelte 笔记三：runtime 源码解读》](https://brandonxiang.top/blog/framework-svelte3)介绍了转换后代码重要的组成部分是 create_fragment，该函数内拥有很多 element 的创建，它会是组件初始化中非常重要的函数，所有的节点都会定义在`src/compiler/compile/nodes`中。组件化是将所有的 Component 组成一个树状结构，render dom 将匹配的节点转换为 runtime 可识别的代码。
